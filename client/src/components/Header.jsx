    ...
    <header className={`fixed left-1/2 -translate-x-1/2 top-5 w-[95%] ${theme === "dark" ? "bg-primary" : "bg-white"} shadow-2xl rounded-xl z-50 transition duration-300`}>
      <nav className="relative container mx-auto flex items-center justify-between py-4 px-6">
        <div className='w-fit flex flex-nowrap items-center'>
          ...
          <button 
            onClick={toggleTheme} 
            className={`bg-transparent w-fit text-2xl ${theme === "dark" ? "text-white" : "text-primary"} hover:text-accent flex flex-nowrap items-center gap-1 py-1 px-2 ml-2 mr-1 border ${theme === "dark" ? "border-white" : "border-primary"} hover:border-accent rounded-lg transition duration-300`}
          >
